...
from alpha.models.Autoformer import Autoformer
...
def main(epoch_num):
    log_file = open("../model/Autoformer/Autoformer_record", mode="a+", encoding="utf-8")
    device = torch.device("cuda")
    model = Autoformer(Configs()).to(device)
    loss_fn = nn.MSELoss()

    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)
    lambda_lr = lambda epoch: max(0.875 ** epoch, 1e-3)
    last_epoch = 0

    # checkpoint = torch.load("../model/backup/Checkpoint_Autoformer.pth")
    ...
    for epoch_ndx in range(last_epoch + 1, epoch_num + 1):
        ...
        torch.save(model, "../model/Autoformer/Autoformer-{:d}.pt".format(epoch_ndx))
        ...
        # }
        # torch.save(checkpoint, "../model/Checkpoint_Autoformer.pth".format(epoch_ndx))
