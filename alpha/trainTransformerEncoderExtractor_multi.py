...
from alpha.ExtractorTransformerEncoder import TransformerEncoderExtractor


def main(epoch_num):
    log_file = open("../model/Encoder/Encoder_record", mode="a+", encoding="utf-8")
    device = torch.device("cuda")
    model = TransformerEncoderExtractor().to(device)
    loss_fn = nn.MSELoss()

    optimizer = torch.optim.AdamW(model.parameters(), lr=5e-4)
    lambda_lr = lambda epoch: max(0.7 ** epoch, 2e-3)
    ...
    for epoch_ndx in range(1, epoch_num + 1):
        ...
        torch.save(model, "../model/Encoder/Encoder-{:d}.pt".format(epoch_ndx))
