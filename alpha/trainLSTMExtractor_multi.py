...
from alpha.ExtractorLSTM import LSTMExtractor


def main(epoch_num):
    log_file = open("../model/LSTM/LSTM_record", mode="a+", encoding="utf-8")
    device = torch.device("cuda")
    model = LSTMExtractor(num_layers=3).to(device)
    loss_fn = nn.MSELoss()
    ...
    for epoch_ndx in range(1, epoch_num + 1):
        ...
        torch.save(model, "../model/LSTM/LSTM-{:d}.pt".format(epoch_ndx))
