...
from alpha.layers.Embed import DataEmbedding, DataEmbedding_wo_pos,DataEmbedding_wo_pos_temp,DataEmbedding_wo_temp
from alpha.layers.AutoCorrelation import AutoCorrelation, AutoCorrelationLayer
from alpha.layers.FourierCorrelation import FourierBlock, FourierCrossAttention
from alpha.layers.MultiWaveletCorrelation import MultiWaveletCross, MultiWaveletTransform
from alpha.layers.SelfAttention_Family import FullAttention, ProbAttention
...
from alpha.layers.Autoformer_EncDec import Encoder, Decoder, EncoderLayer, DecoderLayer, my_Layernorm, series_decomp, series_decomp_multi
import math
import numpy as np
...
class FEDformer(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # self.output_layer = nn.Linear(configs.d_model, configs.num_alphas, bias=True)
        self.flatten = nn.Flatten()
        self.output_layer = nn.Linear(configs.d_model * configs.pred_len, configs.num_alphas, bias=True)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if self.output_attention:
            ...
            dec_out = self.flatten(dec_out[:, -self.pred_len:, :])
