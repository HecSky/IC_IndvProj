...
from alpha.models.Informer import Informer
...
def main(epoch_num):
    log_file = open("../model/Informer/Informer_record", mode="a+", encoding="utf-8")
    device = torch.device("cuda")
    model = Informer(Configs()).to(device)
    loss_fn = nn.MSELoss()

    optimizer = torch.optim.AdamW(model.parameters(), lr=5e-5)
    lambda_lr = lambda epoch: max(0.7 ** epoch, 2e-2)
    last_epoch = 0

    # checkpoint = torch.load("../model/backup/Checkpoint_Informer.pth")
    ...
    for epoch_ndx in range(last_epoch + 1, epoch_num + 1):
        ...
        torch.save(model, "../model/Informer/Informer-{:d}.pt".format(epoch_ndx))
        ...
        # }
        # torch.save(checkpoint, "../model/Checkpoint_Informer.pth".format(epoch_ndx))
