...
class DQN(nn.Module):
    def __init__(self, layer_sizes: list[int]):
        ...
        self.num_actions = layer_sizes[-1]
...
def epsilon_greedy(epsilon: float, dqn: DQN, state: torch.Tensor) -> int:
    ...
    if p > epsilon:
        ...
        return random.randint(0, dqn.num_actions - 1)
...
def loss(policy_dqn: DQN, target_dqn: DQN,
         ...
         rewards: torch.Tensor, next_states: torch.Tensor, dones: torch.Tensor, gamma: float, DDQN: bool = False) -> torch.Tensor:
    ...
    """
    if DDQN:
        policy_action = torch.argmax(policy_dqn(next_states), axis=1).view(-1, 1)
        bellman_targets = (~dones).reshape(-1) * target_dqn(next_states).gather(1, policy_action).view(
            -1) * gamma + rewards.reshape(-1)
        q_values = policy_dqn(states).gather(1, actions).view(-1)
    else:
        bellman_targets = (~dones).reshape(-1) * (target_dqn(next_states)).max(1).values * gamma + rewards.reshape(-1)
        q_values = policy_dqn(states).gather(1, actions).view(-1)
